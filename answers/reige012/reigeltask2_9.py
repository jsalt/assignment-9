...
"""
This program uses various methods to alter the data provided
...
def alter_data():
    # split the data up by line ending, replaces tabs with commas and prints each line
    data = """
    ...
    # replaces the tabs with commas
    newer_data = new_data.split("\n")
    # splits the string by new lines
    newest_data = ''.join(newer_data)
    # joins the list created by the split method back into a single multi-line string
    final_data = newest_data.replace("    ", "\r\n")
    # replaces the 4 spaces separating each bit of info with a new line
    print(final_data)
    print(type(final_data))
    # makes sure the final data is still an immutable string by checking its class
